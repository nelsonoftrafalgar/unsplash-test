import { DynamicAssignProp, IAction } from '../utils/model'
...
export const useGetSinglePhoto = (dispatch: React.Dispatch<IAction>, selecetdPhotoId: string) => {
  useEffect(() => {
    unsplash.photos.getPhoto(selecetdPhotoId)
      .then(toJson)
      .then((data: any) => {
        const singlePhoto: DynamicAssignProp = {}
        ...
        dispatch({type: GET_SINGLE_PHOTO, payload: singlePhoto})
      })
  }, [])
}